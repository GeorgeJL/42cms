...
if(isset($pluginVars['pluginget']['caller']))
{
  switch ($pluginVars['pluginget']['caller']){
    case 'edit_article':
      
      $pos=strlen($config->weburl);
      if(($fileName_b=='.jpg')OR($fileName_b=='.jpeg')OR($fileName_b=='.png')OR($fileName_b=='.bmp')OR($fileName_b=='.tiff')OR($fileName_b=='.gif'))
      {
    ...
    case 'add_images':
      ...
      $thumbnail=true;
      $maxWidth=$config->gallerythumbwidth;
      $maxHeight=$config->gallerythumbheight;
      $quality=90;
      $thumbPath='gallery/'.$_GET['gid'].'/th';
      break;
    default:
      $targetDir='data';
      break;
  ...
  switch($extension)
	{
      ...
      $resizeObj -> saveImage($thumbPath, $quality);
			break;
		default:
			$thumbPath=$fileName;
			break;
  ...
  switch($extension)
	{
		...
		case '.gif':
		case '.png':
			$tinyPath=$tinyPath.'/'.$fileName;
      //$filePath2='';
      include_once('resize-class.php');
      $resizeObj = new resize($filePath);
      $resizeObj -> resizeImage($tinyMaxWidth, $tinyMaxHeight);
      $resizeObj -> saveImage($tinyPath, $tinyQuality);
			break;
		default:
			$tinyPath=$fileName;
			break;
...
//die('{"jsonrpc" : "2.0", "result" : null, "id" : "id", "name" : "'.$fileName.'"}');
die('{"jsonrpc" : "2.0", "result" : null, "id" : "id", "name" : "'.$fileName.'", "debug":"'.$thumbPath.'"     }');
